  ...
  for (const project of activeProjects) {
    ...
    if (completateNonFatt.length > 0) {
      ...
      insights.push({
        ...
        type: 'billing',
        priority: totale > 5000 ? 'urgent' : totale > 1000 ? 'high' : 'medium',
        title: `${completateNonFatt.length} prestazion${completateNonFatt.length === 1 ? 'e' : 'i'} da fatturare`,
        description: `Progetto ${project.code} (${project.client}): ${completateNonFatt.map(p => p.tipo).join(', ')} completat${completateNonFatt.length === 1 ? 'a' : 'e'} per un totale di €${totale.toLocaleString('it-IT', { minimumFractionDigits: 2 })}`,
    ...
    for (const fattura of fattureScadute) {
      ...
      insights.push({
        ...
        description: `Progetto ${project.code}: fattura di €${fattura.importoTotale.toLocaleString('it-IT', { minimumFractionDigits: 2 })} scaduta il ${new Date(fattura.scadenzaPagamento!).toLocaleDateString('it-IT')}`,
        projectId: project.id,
        projectCode: project.code,
        client: project.client,
